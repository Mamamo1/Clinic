...
const InventorySystem = () => {
  ...
  const lowStockMedicineCount = items.filter(
    (item) =>
      ...
      parseInt(item.quantity) <= parseInt(item.threshold || 0)
  ).length;
  ...
  const filtered = items.filter((item) => {
    ...
    if (activeTab === 'Medicine') {
      return (
        (!searchTerm || item.generic?.toLowerCase().includes(searchTerm)) ||
    ...
    <div className="p-6 bg-gray-100 min-h-screen">
      ...
      <div className="flex gap-2 mb-4 items-center">
        {['Medicine', 'Supplies'].map((tab) => {
          const count =
            tab === 'Medicine' ? lowStockMedicineCount : lowStockSuppliesCount;
          return (
            <button
              key={tab}
        ...
        <table className="min-w-full text-sm table-fixed">
          ...
          <tbody>
            {paginated.length === 0 ? (
              <tr>
                <td
                  colSpan={activeTab === 'Medicine' ? 6 : 3}
              ...
              paginated.map((item) => {
                ...
                return (
                  <tr
                    key={item.id}
                    className={`border-b ${isLowStock ? 'bg-red-100' : ''}`}
                  >
                    {activeTab === 'Medicine' ? (
                      <>
